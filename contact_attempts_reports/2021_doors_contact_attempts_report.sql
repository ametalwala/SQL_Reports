SELECT
    ...
    , COUNT(CASE WHEN cc.vb_rank = 1 THEN cc.vanid END) AS total_attempts -- bc there could be people in there 4 times, also do one for vb_voterbase_household_id. try to have all aggregates in one query
    , COUNT(CASE WHEN cc.household_rank = 1 THEN cc.vb_voterbase_household_id END) AS total_doors_attempted 
FROM
(
